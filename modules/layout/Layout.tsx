...
interface LayoutProps {
  currentView: ViewState; onNavigate: (view: ViewState) => void; onLogout: () => void; 
  children: React.ReactNode; notifications: Notification[];
  ...
  return (
    <div className="flex h-screen bg-surface-50 overflow-hidden font-sans">
      ...
      <div className={`flex-1 flex flex-col min-w-0 overflow-hidden relative transition-all duration-300 md:ml-20 ${(isPinned || isHovered) ? 'md:ml-72' : ''}`}>
        ...
        <Header onMobileMenuOpen={() => setIsMobileMenuOpen(true)} onNavigate={onNavigate} onLogout={onLogout} notifications={notifications} onMarkAsRead={onMarkAsRead} onMarkAllAsRead={onMarkAllAsRead} />
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 z-10 scroll-smooth relative">
            <div className="w-full space-y-6 animate-fade-in pb-10">
                {currentView !== 'dashboard' && <Breadcrumbs items={getBreadcrumbs(currentView)} onHome={() => onNavigate('dashboard')} />}
                {children}
            </div>
        </main>
      </div>
