...
export const MobileSidebar: React.FC<Props> = ({ isOpen, onClose, navGroups, onLogout }) => {
    ...
    return (
        <div className={`fixed inset-0 z-50 md:hidden ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
            ...
            <div className={`fixed inset-y-0 left-0 w-72 bg-slate-900 shadow-2xl transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="flex justify-between items-center p-6 border-b border-slate-800">
                    <div className="flex items-center gap-3 text-white font-bold"><div className="p-1 bg-brand-600 rounded"><Package className="w-5 h-5 text-white" /></div>LogiFlow</div>
