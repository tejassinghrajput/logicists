...
export const AddressFieldset: React.FC<Props> = ({ title, data, onChange }) => (
  <div className="space-y-4">
    ...
    <div className="grid grid-cols-2 gap-4">
      <Input label="Name *" value={data.name || ''} onChange={e => onChange({name: e.target.value})} className="col-span-2" />
      ...
      <Input label="Address Line 1 *" value={data.addr1 || ''} onChange={e => onChange({addr1: e.target.value})} className="col-span-2" />
    </div>
  </div>
);