...
export const getOrderColumns = (): Column<OrderLog>[] => [
    { header: 'Channel', accessorKey: 'channel', sortable: true, cell: (row) => {
    ...
    { header: 'Order', accessorKey: 'orderNumber', sortable: true, cell: (row) => <div><div className="font-bold text-slate-900 text-sm hover:text-brand-600 cursor-pointer flex items-center gap-1">{row.orderNumber}<ExternalLink className="w-3 h-3 text-slate-300" /></div><div className="text-xs text-slate-500">{row.productPreview}</div></div>},
    { header: 'Customer', accessorKey: 'customer', sortable: true, cell: (row) => <div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600">{row.customer.charAt(0)}</div><span className="text-sm font-medium text-slate-900">{row.customer}</span></div>},
    { header: 'Value', accessorKey: 'amount', sortable: true, cell: (row) => <span className="text-sm font-bold text-slate-900 font-mono">${row.amount.toFixed(2)}</span>},
    { header: 'Status', accessorKey: 'status', sortable: true, cell: (row) => <StatusCell status={row.status} msg={row.message} />},
    { header: 'Date', accessorKey: 'date', sortable: true, className: 'text-slate-400 text-xs font-medium' },
];