...
export const RegisterForm: React.FC<{ onLogin: () => void; onSignIn: () => void }> = ({ onLogin, onSignIn }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!terms) return toast.error("Terms Required");
    setLoading(true); setTimeout(() => { setLoading(false); onLogin(); }, 1500);
  };
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 animate-slide-up">
        ...
        <Input label="Phone" type="tel" icon={Phone} required />
        <div className="space-y-1"><label className="text-sm font-semibold text-slate-700 ml-0.5">Password</label><div className="relative"><div className="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400"><Lock className="h-5 w-5"/></div><input type="password" className="block w-full rounded-xl border-slate-200 bg-slate-50/50 py-2.5 pl-10 sm:text-sm" placeholder="Create a strong password" required /></div></div>
